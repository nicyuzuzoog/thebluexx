...
import {
  IoAdd, IoTrash, IoEye, IoSearch,
  IoCheckmarkCircle, IoCloseCircle, IoFilter
...
const AdminNews = () => {
  ...
  useEffect(() => { fetchNews(1); }, [filter]);

  const fetchNews = async (page = 1) => {
    setLoading(true);
    try {
      let url = `/admin/news?page=${page}&limit=15`;
  ...
  const handleStatus = async (id, status) => {
    try {
      await api.put(`/admin/content/news/${id}/status`, { status });
  ...
  const filtered = search
    ? news.filter(n =>
      getLocalizedText(n.title, 'en').toLowerCase().includes(search.toLowerCase())
    )
    : news;

  return (
    <AdminLayout title={t('admin.manageNews')}>
      <Helmet><title>{t('admin.manageNews')} — THE BLUEX</title></Helmet>
        ...
        <div className="atb-right">
          <div className="atb-search">
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
              placeholder="Search news..." className="atb-search-input" />
          </div>
        ...
        <table className="at-table">
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan="7" className="at-empty">No news found</td>
              </tr>
            ) : (
              filtered.map((item) => (
                <tr key={item._id}>
                  ...
                  <td className="at-cell-sub">{item.author?.name}</td>
                  <td>
                  ...
                  <td>
                    <div className="at-actions">
                      ...
                      {item.status === 'published' && (
                        <Link to={`/news/${item.slug}`} target="_blank"
      ...
      {pagination.pages > 1 && (
        <div className="pagination" style={{ marginTop: '18px' }}>
      ...
      <style>{`
        ...
        [data-theme="dark"] .filter-pill:hover {
          background: rgba(27,79,255,0.1); color: var(--cobalt-light);
        }
